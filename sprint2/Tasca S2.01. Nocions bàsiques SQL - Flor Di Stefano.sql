...
SELECT trans.id AS id_transaccion
FROM transaction AS trans
WHERE EXISTS 	(	SELECT *
...
FROM transaction AS trans
WHERE trans.declined = 0					-- to filter not declined sales
GROUP BY fecha
...
WHERE trans.company_id IN	(	SELECT comp.id
								FROM company AS comp
								WHERE comp.country =	(	SELECT c.country
															FROM company AS c
...
SELECT comp.company_name AS company_name, comp.phone AS phone, comp.country AS country, DATE(trans.timestamp) AS date, 
SUM(trans.amount) AS amount
FROM company AS comp
INNER JOIN transaction AS trans
ON comp.id = trans.company_id
WHERE DATE(trans.timestamp) IN ('2015-04-29', '2018-07-20', '2024-03-13')
GROUP BY trans.company_id, date
HAVING amount BETWEEN 350 AND 400
ORDER BY amount DESC;


-- MAL!!!
SELECT comp.company_name, comp.phone, comp.country, date, t.cantidad
FROM company AS comp
JOIN 	(	SELECT	trans.company_id, DATE(timestamp) AS date, SUM(trans.amount) AS cantidad
			FROM transaction AS trans
            WHERE cantidad BETWEEN 350 AND 400
...
SELECT trans.company_id AS id_empresa, comp.company_name AS nombre_empresa, COUNT(trans.id) AS total_transacciones,
CASE
